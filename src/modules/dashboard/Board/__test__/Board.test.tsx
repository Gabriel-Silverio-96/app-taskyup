...
import { BOARD_MOCK, CREATE_BOARD_SUCCESS_RESPONSE_MOCK, LIST_BOARD_MOCK_SUCCESS_RESPONSE_MOCK } from "./mock";
...
describe("Component <Board />", () => {
	...
	test("Create new board type notes", async () => {
		mock.onPost("board/create").reply(201, CREATE_BOARD_SUCCESS_RESPONSE_MOCK);
		...
		await waitFor(() => {			
			const snackbarMessage = screen.getByText(CREATE_BOARD_SUCCESS_RESPONSE_MOCK.message);
			expect(snackbarMessage).toBeInTheDocument();

			const boardTitleWasCreated = screen.getByText(LIST_BOARD_MOCK_SUCCESS_RESPONSE_MOCK[0].title);
